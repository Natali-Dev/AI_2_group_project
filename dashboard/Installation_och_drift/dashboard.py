...
def layout(df_installation_och_drift):
    st.title("Installation och Drift")
    
   
    st.header("Övergripande statistik")
    col1, col2, col3 = st.columns(3)
    col1.metric("Totalt antal lediga jobb", total_vacancies(df_installation_och_drift))
    col2.metric("Antal städer", df_installation_och_drift["workplace_city"].nunique())
    col3.metric("Antal arbetsgivare", df_installation_och_drift["employer_name"].nunique())

    # Most requested occupations
    st.header("Mest efterfrågade yrken")
    top_occ = top_occupations(df_installation_och_drift)
    ...
    city_vacancies = vacancies_per_city(df_installation_och_drift)
    st.plotly_chart(bar_chart(city_vacancies, "Stad", "Antal jobb", "Lediga jobb per stad"))

    # Pie chart for occupational groups

    if "occupation_group" in df_installation_och_drift:
        st.header("Fördelning av yrkesgrupper")
        occupations_group_counts_df = occupations_group_counts(df_installation_och_drift)
    ...
    st.plotly_chart(map_chart(df_installation_och_drift))

    # Attributes required for jobs
    st.header("Krav för anställning")
    attributes_df = get_attributes_per_field(df_installation_och_drift)
...
if __name__ == "__main__":
    ...
    try:
        with duckdb.connect("ads_data.duckdb") as connection:
            df_installation_och_drift = connection.execute("SELECT * FROM mart.mart_installation_drift_underhall").df()
    except Exception as e:
        st.error(f"Failed to load data from DuckDB: {e}")
        st.stop()  
    layout(df_installation_och_drift)