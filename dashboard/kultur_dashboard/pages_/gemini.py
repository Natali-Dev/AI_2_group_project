...
import duckdb
from chart_kpi import df_kultur
...
def gemini_layout():
    ...
    category = st.selectbox("Välj en kategori:", ["Workplace City","Employer Name", "Occupation"]) 
    choice_sort = st.selectbox("välj en stad/arbetsgivare/yrke", df_kultur[category].sort_values().unique())
    choice_question = st.selectbox("Välj en fråga", ["Select...",
    ...
    if choice_question != 'Select...':
        text = gemini_logic(category, choice_sort, choice_question)
        st.write(text)
...
def gemini_logic(category, choice_sort, choice_question):
    top_employer = df_kultur.groupby(category)[["Vacancies", "Description"]].sum().sort_values(by="Vacancies", ascending=False).reset_index()
